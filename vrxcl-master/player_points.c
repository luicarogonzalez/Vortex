...
void check_for_levelup(edict_t *ent)
{
	...
	int plateau_points = Lua_GetIntVariable("PlateauPoints", 3000);
	...
	while (ent->myskills.experience >= ent->myskills.next_level)
	{
		...
		if (ent->myskills.level > 10)
		{
			ent->myskills.next_level += points_needed + plateau_points/3 * (ent->myskills.level);
		}
...
int PVP_AwardKill(edict_t *attacker, edict_t *targ, edict_t *target)
{
	...
	else
	{
		...
		{
		case 0:			experienceFactor = 7.0;				break;
		case 1:			experienceFactor = 6.6;				break;
		case 2:			experienceFactor = 6.2;				break;
		case 3:			experienceFactor = 5.8;				break;
		case 4:			experienceFactor = 5.2;				break;
		case 5:			experienceFactor = 4.5;				break;
		case 6:			experienceFactor = 3.0;				break;
		case 7:			experienceFactor = 2.4;				break;
		case 8:			experienceFactor = 2.1;				break;
		case 9:			experienceFactor = 1.8;				break;
		case 10:		experienceFactor = 1.5;				break;
		case 11:		experienceFactor = 1.4;				break;
		case 12:		experienceFactor = 1.3;				break;
		case 13:		experienceFactor = 1.2;				break;
		case 14:		experienceFactor = 1.0;				break;
	...
	if (!attacker->ai.is_bot)
	{
		...
		if (IsTalentActive(attacker, TALENT_DUAL_LEECH))
		{
			...
			{
				total_life_gained = 1 + attacker->myskills.level;
			}
			
			if (attacker->client->pers.inventory[power_cube_index] >= attacker->max_health)
			{
				total_mana_gained = 1;
			}
			else
			{
				total_mana_gained = 1 + attacker->myskills.level;
			}
			...
			attacker->health += total_life_gained;
			attacker->client->pers.inventory[power_cube_index] += total_mana_gained;
