...
void AssignAbilities(edict_t *ent)
{
	...
	// enable general skills: first->general == 1
	while (first->index != -1)
	{
		if (first->general == 1)
		{
	...
	if (ent->myskills.class_num == CLASS_WEAPONMASTER || generalabmode->value)
	{
		...
		for (i = 0; i < MAX_ABILITIES; i++)
		{
			...
			if (first)
			{
				...
				if (first->softmax > 20) // a 15 softmax? dump down to 8
